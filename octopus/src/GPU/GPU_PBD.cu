...
__global__ void kernel_step_solver(const int n, const float dt, const Vector3 g, Vector3* p, Vector3* prev_p, Vector3* v, Vector3* f, float* w) {
    const int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i >= n) return;
    prev_p[i] = p[i];
    v[i] += (g + f[i] * w[i])*dt;
...
__device__ void print_mat(const Matrix3x3& m) {
    printf("%f %f %f %f %f %f %f %f %f", m[0][0], m[1][0],m[2][0],m[0][1],m[1][1],m[2][1],m[0][2],m[1][2],m[2][2]);
}
...
__global__ void kernel_constraint_solve(
    ...
    const Vector3* cb_dN, // Derivative of shape function
    ...
    scalar young = 1e7f; scalar poisson = 0.35f;
    ...
    for(int m = 0; m < 2; ++m) {
        ...
        for(int q = 0; q < nb_quadrature; ++q) {
            ...
            if(m == 0) {
                ...
                P = 2.f * F * E;
                C += (E[0][0] * E[0][0] + E[1][1] * E[1][1] + E[2][2] * E[2][2]) + (E[0][1] * E[1][0] + E[1][2] * E[2][1] + E[2][0] * E[0][2]) * 2.f;
            }
        ...
        if(sum_norm_grad < 1e-12f) continue;
        scalar stiffness = m == 0 ? lambda : mu * 2.f;
...
void GPU_PBD_FEM::step(const scalar dt) const {
    ...
    for(int i = 0; i < sub_it; ++i) {
       ...
       cb_position->buffer, cb_prev_position->buffer, cb_velocity->buffer, cb_forces->buffer, cb_inv_mass->buffer);

        for(int j = 0; j < c_offsets.size(); ++j) {
            kernel_constraint_solve<<<c_nb_elem[j],256>>>(c_nb_elem[j], nb_quadrature, elem_nb_vert, sub_dt,
                c_offsets[j],
                cb_dN->buffer,
        ...
        kernel_constraint_plane<<<(cb_position->nb+255)/256, 256>>>(cb_position->nb, Unit3D::right() * 0.01f, -Unit3D::right(),
