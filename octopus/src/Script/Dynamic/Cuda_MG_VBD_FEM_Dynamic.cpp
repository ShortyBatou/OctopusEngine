...
{
    return new GPU_MG_VBD(_mesh->geometry(), get_fem_masses(), _iteration, _sub_iterations, _rho);
}
...
void Cuda_MG_VBD_FEM_Dynamic::build_dynamics()
{
    for(auto&[e, topo] : _mesh->topologies()) {
        ...
        GPU_FEM* fem = new GPU_MG_VBD_FEM(e, topo, _mesh->geometry(), _material, _young, _poisson, _damping, _linear, _iteration);
        _gpu_fems[e] = fem;
