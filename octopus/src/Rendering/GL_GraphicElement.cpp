...
GL_GraphicElement::GL_GraphicElement(const Color& color, const scalar scale) : GL_Graphic(color), _scale(scale)
{
...
void GL_GraphicElement::update_gl_topology()
{
...
void GL_GraphicElement::update_gl_vcolors()
{
    _gl_geometry->vcolors.clear();
    for (const auto& it : _mesh->topologies())
    {
        const int size = static_cast<int>(_gl_geometry->vcolors.size());
