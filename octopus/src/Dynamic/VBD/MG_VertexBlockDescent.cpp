...
MG_VBD_FEM::MG_VBD_FEM(const Mesh::Topology &topology, const Mesh::Geometry &geometry, FEM_Shape *shape,
                 FEM_ContinuousMaterial *material) : _shape(shape), _material(material), _topology(topology) {
...
void MG_VBD_FEM::build_fem_const(const Mesh::Topology &topology, const Mesh::Geometry &geometry) {
...
void MG_VBD_FEM::solve(ParticleSystem *ps, const scalar dt) {
...
void MG_VBD_FEM::plot_residual(ParticleSystem *ps, const scalar dt) {
    // compute error
    const int nb_vertices = static_cast<int>(_owners.size());
...
scalar MG_VBD_FEM::compute_energy(ParticleSystem *ps) const {
    scalar energy = 0;
...
std::vector<Vector3> MG_VBD_FEM::compute_forces(ParticleSystem *ps, const scalar dt) const {
...
void MG_VBD_FEM::solve_vertex(ParticleSystem *ps, const scalar dt, const int vid) {
...
void MG_VBD_FEM::solve_element(ParticleSystem *ps, const int eid, const int ref_id, Vector3 &f_i, Matrix3x3 &H_i) {
    const int nb_quadrature = _shape->nb_quadratures();
    const int nb_vert_elem = _shape->nb;
...
void MG_VBD_FEM::compute_inertia(ParticleSystem *ps, const scalar dt) {
    for (int i = 0; i < ps->nb_particles(); ++i) {
        Particle *p = ps->get(i);
        if (p->active)
