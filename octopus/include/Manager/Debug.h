...
void Debug::Cube(const Vector3& p_min, const Vector3 p_max)
{
    ...
    static int box_topo[24] = { 0, 1, 1, 3, 3, 2, 2, 0, 2, 6, 6, 7,
...
struct DebugUI_Plot : public DebugUI_Component {
    DebugUI_Plot(std::string name, int size, bool auto_range = true)
    ...
    virtual void draw(int w, int h) {
        ImGui::PlotLines("", _values.data(), _size, _offset, "", _r_min, _r_max, ImVec2(w * 0.8f, 60));
    }
...
public:
    virtual void draw() {
        ...
        ImGui::SetNextWindowPos(ImVec2(w-410.f,10.f));
        int sx = 400;
