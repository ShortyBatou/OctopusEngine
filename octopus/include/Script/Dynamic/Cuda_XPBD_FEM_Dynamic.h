...
struct Cuda_XPBD_FEM_Dynamic final : Cuda_FEM_Dynamic
{
    explicit Cuda_XPBD_FEM_Dynamic(
        ...
        const int sub_it = 30, const scalar damping = 0.f, const bool coupled = false, XPBD_FEM_VERSION _version = XPBD_FEM_VERSION::OptiShared)
        : Cuda_FEM_Dynamic(sub_it, density, m_distrib, young, poisson, material, damping),
            _coupled_fem(coupled), version(_version)
    { }
