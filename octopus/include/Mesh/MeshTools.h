...
void MeshTools::GetSurface(
    ...
    const int nb_id_per_element = static_cast<int>(ref_topology.size());
...
void MeshTools::GetSurface(
    ...
    int nb_id_per_element = static_cast<int>(ref_topology.size());
    for (int i = 0; i < topology.size(); i += NB) {
        int element_id = i / nb_id_per_element;
        int face_id = (i % nb_id_per_element) / NB;
        for (int j = 0; j < NB; ++j) {
...
/// removes duplicate between two face sets
template<int NB>
void MeshTools::RemoveDuplicates(std::set<Face<NB> > &a_faces, std::set<Face<NB> > &b_faces) {
...
/// convert the face set into an indices array
template<int NB>
void MeshTools::ExtractTopo(const std::set<Face<NB> > &faces, Mesh::Topology &topology) {
...
/// gives the map between a face and its element id
template<int NB>
void MeshTools::ExtractFaceToElem(const std::set<Face<NB> > &faces, Mesh::Topology &face_to_elem) {
    const int f_size = static_cast<int>(face_to_elem.size());
...
// get the map face -> element from elements topo
template<int NB>
void MeshTools::ExtractTopoToElem(const Mesh::Topology &topo, const Mesh::Topology &ref_topology,
                                  Mesh::Topology &face_to_elem) {
    const int nb_id_per_element = static_cast<int>(ref_topology.size());
...
// get the geometry according to faces, give a map vertices -> element and old to new ids
template<int NB>
void MeshTools::RebuildFaces(std::set<Face<NB> > &faces, Mesh::Geometry &geometry, std::vector<int> &v_to_elem,
                             std::map<int, int> &map_id) {
    ...
    for (const Face<NB> &face: faces) {
        for (int i = 0; i < NB; ++i) {
            ...
            // if not found
            if (it == map_id.end()) {
                map_id[id] = static_cast<int>(geometry.size());
    ...
    for (const Face<NB> &face: faces) {
        ...
        for (int j = 0; j < subdivision_edge.size(); j += 2) {
            ...
            if (edges.find(edge) == edges.end()) {
                cid = static_cast<int>(refined_geometry.size());
...
template<int nb>
void MeshTools::TryAddFace(std::set<Face<nb> > &faces, Face<nb> &face, const bool remove_duplicates) {
