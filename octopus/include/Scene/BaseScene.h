...
#include "Script/Dynamic/Cuda_FEM_Explicit_Dynamic.h"
...
struct BaseScene final : Scene
{
    ...
    void build_entities() override
    {
        ...
        build_xpbd_entity(Vector3(0,0,0), cells, size, Color(0.8,.2,0.8,0), elem, args, true, false, XPBD_FEM_VERSION::OptiGroup);
    ...
    Mesh* build_vtk_mesh(const Vector3& pos, const std::string& file, const Element element, const SimulationArgs& args) {
        ...
        std::cout << "Sommet " << mesh->nb_vertices() << std::endl;
        return mesh;
    }
    ...
    void build_fem_entity(const Vector3& pos, const Vector3I& cells, const Vector3& size, const Color& color, const Element element, const SimulationArgs& args, bool gpu, IntegratorType integrator) {
        Entity* e = Engine::CreateEnity();
        add_fem_mesh(e, pos, cells, size, element, args);
        if(gpu) e->add_component(new Cuda_FEM_Explicit_Dynamic(std::max(args.iteration, args.sub_iteration), args.density, args.distribution, args.young, args.poisson, args.material, args.damping, integrator));
    ...
    void build_xpbd_entity(const Vector3& pos, const Vector3I& cells, const Vector3& size, const Color& color, const Element element, const SimulationArgs& args, const bool gpu, const bool coupled, const XPBD_FEM_VERSION version) {
        Entity* e = Engine::CreateEnity();
        add_fem_mesh(e, pos, cells, size, element, args);
        if(gpu) e->add_component(new Cuda_XPBD_FEM_Dynamic(args.density, args.distribution, args.young, args.poisson,args.material,std::max(args.iteration, args.sub_iteration), args.damping,coupled, version));
